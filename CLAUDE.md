...
- OAuth callback handled at `/callback` route
...
| File | Purpose |
|------|---------|
| `src/App.tsx` | Routes: `/login`, `/`, `/generate` with protected routes |
...
- `/` - Dashboard with contribution calendar (protected)
- `/generate` - Pattern designer and commit generator (protected)
- `/callback` - OAuth callback handler

### Data Flow
1. User logs in via GitHub OAuth → token stored in localStorage
2. Dashboard fetches contributions via GraphQL API
3. PatternDesigner maps pattern grid to dates, creates commits via Git API
